...
from PyQt6.QtWidgets import (
    ...
    QToolButton, QMenu, QDialog, QSlider, QStackedLayout
)
...
from PyQt6.QtGui import (
    ...
    QCursor, QFont, QColor, QPixmap, QClipboard, QAction
    )
...
class FloatingPropertiesDialog(QDialog):
    def __init__(self, parent=None, undo_stack=None, selected_shape_index=None):
        ...
        bg_layout.setContentsMargins(18, 22, 18, 12)
        bg_layout.setSpacing(10)
        
        # Top drag bar
        drag_bar = QWidget(self.bg)
        drag_bar.setFixedSize(48, 5)
        drag_bar.setStyleSheet("""
            background: #222;
            border-radius: 44px;
        """)
        drag_bar.setCursor(Qt.CursorShape.SizeAllCursor)
        drag_bar.mousePressEvent = self.mousePressEvent
        drag_bar.mouseMoveEvent = self.mouseMoveEvent
        drag_bar.mouseReleaseEvent = self.mouseReleaseEvent
        bg_layout.addWidget(drag_bar, alignment=Qt.AlignmentFlag.AlignHCenter)
        ...
        # Border Radius row (use CustomSlider)
        border_radius_col = QVBoxLayout()
        border_radius_label = QLabel("Border Weight")
        border_radius_label.setFixedHeight(28)
        border_radius_label.setStyleSheet("""
            font-size: 18px;
            font-weight: bold;
            border: none;
            margin-top: 0px;
            margin-bottom: 0px; 
        """)
        border_radius_col.addWidget(border_radius_label)
        self.radius_slider = CustomSlider(0, 20, 2)
        
        self.radius_slider.setFixedWidth(310)
        self.radius_slider.setFixedHeight(40)
        self.radius_slider.setStyleSheet("""
            margin-top: 0px;
        """)
        border_radius_col.addWidget(self.radius_slider, alignment=Qt.AlignmentFlag.AlignHCenter)
        bg_layout.addLayout(border_radius_col)

        # Border Color row
        border_color_col = QVBoxLayout()
        border_color_col.setSpacing(8)
        border_color_label = QLabel("Border Color")
        border_color_label.setFixedHeight(40)
        border_color_label.setStyleSheet("""
            ...
            margin-bottom: 0px; 
        """)
        border_color_col.addWidget(border_color_label)
        bg_layout.addLayout(border_color_col)
        
        # Color Horizontal Layout
        color_row = QHBoxLayout()
        color_row.setSpacing(1)
        for color in ["#000", "#f66", "#fa0", "#fc0", "#0c0", "#0cf"]:
            btn = QPushButton(self.bg)
            ...
            color_row.addWidget(btn, alignment=Qt.AlignmentFlag.AlignTop)
        pipette = QLabel("🧪", self.bg)
        pipette.setFixedSize(25, 25)
        pipette.setAlignment(Qt.AlignmentFlag.AlignHCenter)
        color_row.addWidget(pipette)
        bg_layout.addLayout(color_row)
        

        # Object Color row
        object_color_col = QVBoxLayout()
        object_color_col.setSpacing(8)
        object_color_label = QLabel("Object Color")
        object_color_label.setFixedHeight(40)
        object_color_label.setStyleSheet("""
            font-size: 18px; 
            font-weight: bold;
            border: none;
            margin-top: 0px;
            margin-bottom: 0px; 
        """)
        object_color_col.addWidget(object_color_label)
        bg_layout.addLayout(object_color_col)
        
        
        # Color Horizontal Layout
        color2_row = QHBoxLayout()
        color2_row.setSpacing(1)
        for color in ["#000", "#f66", "#fa0", "#fc0", "#0c0", "#0cf"]:
            btn = QPushButton(self.bg)
            btn.setFixedSize(25, 25)
            btn.setStyleSheet(f"""
            ...
            """)
            btn.setCursor(Qt.CursorShape.PointingHandCursor)
            color2_row.addWidget(btn, alignment=Qt.AlignmentFlag.AlignTop)
        pipette2 = QLabel("🧪")
        pipette2.setFixedSize(25, 25)
        pipette2.setAlignment(Qt.AlignmentFlag.AlignHCenter)
        color2_row.addWidget(pipette2)
        bg_layout.addLayout(color2_row)

        # Restore Point link
        restore_btn = QPushButton("Restore Point")
        ...
        """)
        restore_btn.setFixedHeight(50)
        restore_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        restore_btn.clicked.connect(self.show_restore_points_view)
        bg_layout.addWidget(restore_btn, alignment=Qt.AlignmentFlag.AlignLeft)
        

        self.stacked.addWidget(self.bg)
        
        # Restore Point
        self.restore_widget = QWidget(self)
        self.restore_widget.setStyleSheet("""
            background: white;
            border-radius: 18px;
            border: 2px solid #222;
        """)
        restore_layout = QVBoxLayout(self.restore_widget)
        restore_layout.setContentsMargins(18, 26, 18, 12)
        restore_layout.setSpacing(10)
        
        # Top drag bar
        drag_bar_2 = QWidget(self.bg)
        drag_bar_2.setFixedSize(48, 5)
        drag_bar_2.setStyleSheet("""
            background: #222;
            border-radius: 44px;
        """)
        drag_bar_2.setCursor(Qt.CursorShape.SizeAllCursor)
        drag_bar_2.mousePressEvent = self.mousePressEvent
        drag_bar_2.mouseMoveEvent = self.mouseMoveEvent
        drag_bar_2.mouseReleaseEvent = self.mouseReleaseEvent
        restore_layout.addWidget(drag_bar_2, alignment=Qt.AlignmentFlag.AlignHCenter)

        # Properties label
        label_2 = QLabel("Properties")
        ...
        label_2.setStyleSheet("""
            ...
            margin-top: 10px;
        """)
        label_2.setFixedHeight(40)
        restore_layout.addWidget(label_2)

        # Top row: back arrow + label
        top_row = QHBoxLayout()
        ...
        restore_label = QLabel("Restore Points")
        restore_label.setStyleSheet("""
            ...
            border: none;
        """)
        top_row.addWidget(restore_label)
        restore_layout.addLayout(top_row)

        # List of restore points
        self.restore_points_container = QVBoxLayout()
        restore_layout.addLayout(self.restore_points_container)
        restore_layout.addStretch()
        self.stacked.addWidget(self.restore_widget)

        self.show_main_view()
        
    def show_restore_points_view(self):
        # Clear previous restore point buttons
        while self.restore_points_container.count():
            item = self.restore_points_container.takeAt(0)
            widget = item.widget()
            if widget:
                widget.deleteLater()
        # Populate with up to 5 recent versions
        versions = self.get_recent_versions_for_selected_object()
        for i, v in enumerate(versions):
            btn = QPushButton(f"[{v['timestamp']}] {v['label']}")
            btn.setStyleSheet("""
                QPushButton {
                    text-align: left;
                    font-size: 13px;
            ...
            """)
            btn.clicked.connect(lambda checked, version=v: self.restore_version(version))
            self.restore_points_container.addWidget(btn)
        self.stacked.setCurrentWidget(self.restore_widget)
    
    def show_main_view(self):
        self.stacked.setCurrentWidget(self.bg)
    
    def get_recent_versions_for_selected_object(self):
        # Find previous versions of the selected object in the undo stack
        versions = []
        idx = self.selected_shape_index
        count = 0
        for state in reversed(self.undo_stack):
            shapes = state[0]
            timestamp = state[4] if len(state) > 4 else "unknown"
    ...
    def paintEvent(self, event):
        # Draw drag bar and shadow
        painter = QPainter(self)
        painter.setRenderHint(QPainter.RenderHint.Antialiasing)
        # Shadow
        rect = QRect(0, 18, self.width(), self.height() - 18)
        painter.setBrush(QColor(0, 0, 0, 40))
        painter.setPen(Qt.PenStyle.NoPen)
        painter.drawRoundedRect(rect.adjusted(2, 2, -2, -2), 18, 18)
        # Drag bar line
        painter.setPen(QPen(QColor("#222"), 3, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
        painter.drawLine(self.width()//2 - 22, 9, self.width()//2 + 22, 9)
        painter.end()
        super().paintEvent(event)

    def mousePressEvent(self, event: QMouseEvent):
        if event.button() == Qt.MouseButton.LeftButton and event.position().y() < 18:
            self._dragging = True
...
class DrawingArea(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        self.crop_preview = False
        self.crop_rect = None
        self._dragging_crop_handle = None
        self._dragging_crop = False
        self.crop_drag_offset = None
        self._orig_crop_rect = None
    ...
    def mousePressEvent(self, event):
        ...
        if getattr(self, "crop_preview", False):
            if self.crop_rect is None:
                return
            pt = self.widget_to_canvas(event.position().toPoint())
            # Check handles for resizing
            for idx, handle in enumerate(self.handle_points(self.crop_rect)):
                handle_rect = QRect(handle.x() - self.HANDLE_SIZE // 2, handle.y() - self.HANDLE_SIZE // 2, self.HANDLE_SIZE, self.HANDLE_SIZE)
                if handle_rect.contains(pt):
                    self._dragging_crop_handle = idx
                    self._orig_crop_rect = QRect(self.crop_rect)
                    return
            # Check for moving
            if self.crop_rect.contains(pt):
                self._dragging_crop = True
                self.crop_drag_offset = pt - self.crop_rect.topLeft()
                return
        ...
        if self._free_rotating and self.selected_shape_index is not None:
            ...
            if self._rotation_initial is None:
                self._rotation_initial = 0
            return
        ...
        if self.preview_shape and not isinstance(self.preview_start, list) and not isinstance(self.preview_end, list):
            ...
            if self.selected_shape_index is not None:
                ...
                if self.preview_shape == "image":
                    ...
                    if hasattr(self, 'preview_fill_color') and self.preview_fill_color:
                        new_shape = (*new_shape, self.preview_fill_color)
                    self.shapes[self.selected_shape_index] = self.normalize_shape_tuple(new_shape)
            else:
                self.push_undo()
                ...
                if self.preview_shape == "image":
                    ...
                    if self.preview_fill_color:
                        new_shape = (*new_shape, self.preview_fill_color)
                    self.shapes.append(self.normalize_shape_tuple(new_shape))
            ...
            if self.canvas_contains(pt):
                if self.preview_shape is None:
                    ...
                    if self.preview_shape == "image":
                        ...
                        self.shapes.append(self.normalize_shape_tuple((self.preview_shape, self.preview_start, self.preview_end, self.shape_color)))
                        self.preview_shape = None
                        self.preview_start = None
                        self.preview_end = None
                self.update()
    ...
    def mouseMoveEvent(self, event):
        ...
        if getattr(self, "crop_preview", False):
            pt = self.widget_to_canvas(event.position().toPoint())
            if getattr(self, "_dragging_crop_handle", None) is not None:
                idx = self._dragging_crop_handle
                rect = self._orig_crop_rect
                x1, y1, x2, y2 = rect.left(), rect.top(), rect.right(), rect.bottom()
                if idx == 0:  # Top-left
                    self.crop_rect = QRect(pt, QPoint(x2, y2)).normalized()
                elif idx == 1:  # Top-middle
                    self.crop_rect = QRect(QPoint(x1, pt.y()), QPoint(x2, y2)).normalized()
                elif idx == 2:  # Top-right
                    self.crop_rect = QRect(QPoint(x1, pt.y()), QPoint(pt.x(), y2)).normalized()
                elif idx == 3:  # Right-middle
                    self.crop_rect = QRect(QPoint(x1, y1), QPoint(pt.x(), y2)).normalized()
                elif idx == 4:  # Bottom-right
                    self.crop_rect = QRect(QPoint(x1, y1), pt).normalized()
                elif idx == 5:  # Bottom-middle
                    self.crop_rect = QRect(QPoint(x1, y1), QPoint(x2, pt.y())).normalized()
                elif idx == 6:  # Bottom-left
                    self.crop_rect = QRect(QPoint(pt.x(), y1), QPoint(x2, pt.y())).normalized()
                elif idx == 7:  # Left-middle
                    self.crop_rect = QRect(QPoint(pt.x(), y1), QPoint(x2, y2)).normalized()
                self.update()
                return
            elif getattr(self, "_dragging_crop", False):
                offset = pt - self.crop_drag_offset
                size = self.crop_rect.size()
                self.crop_rect = QRect(offset, size)
                self.update()
                return
        
        if getattr(self, "cropping", False):
            self.crop_end = pt
            self.update()
            return
    ...
    def mouseReleaseEvent(self, event):
        ...
        if self.preview_shape == "draw" and isinstance(self.preview_start, list):
            ...
            if self.selected_shape_index is not None:
                self.push_undo()
                self.shapes[self.selected_shape_index] = self.normalize_shape_tuple((
                    self.preview_shape,
                    self.preview_start,
                    self.preview_end,
                    self.shape_color,
                    getattr(self, 'preview_rotation', 0),
                    self.shapes[self.selected_shape_index][5]
                ))
            else:
                # Commit new/pasted freehand shape
                if self.preview_start and len(self.preview_start) > 1:
                    self.push_undo()
                    self.shapes.append(self.normalize_shape_tuple((
                        "draw",
                        [QPoint(p) for p in self.preview_start],
                        None,
                        self.shape_color,
        ...
        if self.current_tool == "draw" and getattr(self, "drawing", False):
            ...
            if hasattr(self, "freehand_points") and len(self.freehand_points) > 1:
                self.push_undo()
                self.shapes.append(self.normalize_shape_tuple(("draw", self.freehand_points[:], None, self.shape_color, self.draw_radius)))
    ...
    def paintEvent(self, event):
        ...
        for idx, shape in enumerate(self.shapes):
            ...
            fill_color = shape[5] if len(shape) > 5 and isinstance(shape[5], QColor) else None
            rotation = 0
            if len(shape) > 4 and isinstance(shape[4], (int, float)):
                rotation = shape[4]
            elif len(shape) > 5 and isinstance(shape[5], (int, float)):
                rotation = shape[5]
            ...
            if tool == "image" and isinstance(data, QPixmap):
                ...
                # For lines, use fill_color as pen if present
                if tool == "line" and fill_color:
                ...
                if tool == "draw":
                    ...
                    # Use fill_color as pen if present, else use color
                    pen_color = fill_color if fill_color else color
                    painter.setPen(QPen(pen_color, int(draw_width), Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap, Qt.PenJoinStyle.RoundJoin))
                    painter.setBrush(Qt.BrushStyle.NoBrush)
        ...
        if getattr(self, "cropping", False) or getattr(self, "crop_preview", False):
            # Draw dark overlay except crop rect
            crop_rect = QRect(self.crop_start, self.crop_end).normalized() if getattr(self, "cropping", False) else self.crop_rect
            if crop_rect:
                # Get the A4 canvas rect in widget coordinates
                canvas_widget_rect = self.canvas_to_widget_rect(QRect(QPoint(0, 0), self.a4_size))
                # Get the crop rect in widget coordinates
                crop_widget_rect = self.canvas_to_widget_rect(crop_rect)
                # Draw semi-transparent overlay over the canvas area, except the crop rect
                painter.resetTransform()
                painter.save()
                painter.setBrush(QColor(0, 0, 0, 100))
                painter.setPen(Qt.PenStyle.NoPen)
                # Draw the overlay as four rectangles around the crop area, clipped to the canvas
                r = canvas_widget_rect
                c = crop_widget_rect.intersected(canvas_widget_rect)
                # Top
                if c.top() > r.top():
                    painter.drawRect(r.left(), r.top(), r.width(), c.top() - r.top())
                # Bottom
                if c.bottom() < r.bottom():
                    painter.drawRect(r.left(), c.bottom() + 1, r.width(), r.bottom() - c.bottom())
                # Left
                if c.left() > r.left():
                    painter.drawRect(r.left(), c.top(), c.left() - r.left(), c.height())
                # Right
                if c.right() < r.right():
                    painter.drawRect(c.right() + 1, c.top(), r.right() - c.right(), c.height())
                # Draw crop rectangle border and handles (in widget coordinates)
                painter.setPen(QPen(QColor("#ff6600"), 2, Qt.PenStyle.DashLine))
                painter.setBrush(Qt.BrushStyle.NoBrush)
                painter.drawRect(c)
                for pt in self.handle_points(crop_rect):
                    widget_pt = self.canvas_to_widget(pt)
                    painter.setBrush(QColor("white"))
                    painter.setPen(QPen(QColor("#ff6600"), 2))
                    painter.drawRect(widget_pt.x() - self.HANDLE_SIZE//2, widget_pt.y() - self.HANDLE_SIZE//2, self.HANDLE_SIZE, self.HANDLE_SIZE)
                painter.restore()
    ...
    def keyPressEvent(self, event):
        ...
        if self.preview_shape == "image" and (event.key() == Qt.Key.Key_Return or event.key() == Qt.Key.Key_Enter):
            self.push_undo()
            self.shapes.append((
                "image",
                self.preview_start,
    ...
    def select_shape_by_index(self, idx):
        ...
        if tool == "image":
            ...
            self.preview_end = end
            self.preview_rotation = self.shapes[idx][4] if len(self.shapes[idx]) > 4 else 0
            self.preview_fill_color = self.shapes[idx][5] if len(self.shapes[idx]) > 5 else None
            self.preview_pixmap = None
            self.shape_color = data
    ...
    def contextMenuEvent(self, event):
        ...
        if can_rotate and self.preview_shape == "draw":
            can_rotate = False
        ...
        properties_action.setEnabled(can_delete)
        def show_properties_dialog():
            dlg = FloatingPropertiesDialog(self,
                undo_stack=self.undo_stack,
                selected_shape_index=self.selected_shape_index)
            # Optionally, center dialog near mouse or shape
            dlg.move(event.globalPos() - dlg.rect().center())
            dlg.show()
            dlg.raise_()
        properties_action.triggered.connect(show_properties_dialog)
    ...
    def start_free_rotate(self):
        ...
        if idx is not None and 0 <= idx < len(self.shapes):
            ...
            # For preview shape
            if self.preview_shape and len(shape) == 4:
                self.preview_rotation = 0
            else:
    ...
    def rotate_selected_shape(self):
        ...
        # Save current state for undo AFTER mutation
        old_shape = self.shapes[idx]
        ...
        if tool == "draw" and isinstance(start, list) and len(start) > 1:
            ...
            rotation = self.shapes[idx][5] if len(self.shapes[idx]) > 5 else (self.shapes[idx][4] if len(self.shapes[idx]) > 4 else 0)
            draw_radius = self.shapes[idx][4] if len(self.shapes[idx]) > 4 else self.draw_radius
            if len(self.shapes[idx]) > 4:
                self.shapes[idx] = (tool, rotated_points, None, color, draw_radius, (rotation + 90) % 360)
            else:
                self.shapes[idx] = (tool, rotated_points, None, color, draw_radius)
        ...
        if (
            ...
            and self.preview_start is not None
    ...
    def push_undo(self):
        # Deep copy all shapes and their fields
        def deep_copy_shape(shape):
            tool, start, end, data = shape[:4]
            # Deep copy start/end
            if tool == "image":
                # QPixmap: shallow copy reference is fine
                new_shape = [tool, QPoint(start), QPoint(end), data]
            elif tool == "draw":
                # Copy the list of points
                points_copy = [QPoint(p.x(), p.y()) for p in start] if isinstance(start, list) else []
                new_shape = [tool, points_copy, None, QColor(data)]
            else:
                new_shape = [tool, QPoint(start), QPoint(end), QColor(data)]
            # Copy extra fields (rotation, fill, etc.)
            for i in range(4, len(shape)):
                field = shape[i]
                if isinstance(field, QColor):
                    new_shape.append(QColor(field))
                elif isinstance(field, list):
                    new_shape.append(copy.deepcopy(field))
                else:
                    new_shape.append(copy.deepcopy(field))
            return tuple(new_shape)

        new_shapes = [deep_copy_shape(shape) for shape in self.shapes]        
        ...
        ui_state = {
            ...
            "preview_pixmap": self.preview_pixmap,
            "preview_fill_color": self.preview_fill_color,
            "editing": self.preview_shape is not None,
            "selected_shape_intentional": self.selected_shape_index is not None
        }
        
        timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%SH")
        self.undo_stack.append((new_shapes, eraser_mask_copy, eraser_strokes_copy, ui_state, timestamp))
    ...
    def undo(self):
        if self.undo_stack:
            ...
            self.redo_stack.append((self.shapes, self.eraser_mask.copy(), [list(s) for s in self.eraser_strokes], current_ui_state))
        
            # Load previous state
            shapes, eraser_mask, eraser_strokes, ui_state = self.undo_stack.pop()
            self.shapes = shapes
            self.eraser_mask = eraser_mask
            self.eraser_strokes = [list(s) for s in eraser_strokes]
        
            # Restore UI state
            if ui_state["selected_shape_intentional"]: # Add this condition
                self.selected_shape_index = ui_state["selected_shape_index"]
            else:
                self.selected_shape_index = None

            if ui_state["editing"]:  # Add this condition
                self.preview_shape = ui_state["preview_shape"]
                self.preview_start = ui_state["preview_start"]
                self.preview_end = ui_state["preview_end"]
                self.preview_pixmap = ui_state["preview_pixmap"]
                self.preview_fill_color = ui_state["preview_fill_color"]
            else:
                self.preview_shape = None
                self.preview_start = None
                self.preview_end = None
    ...
    def redo(self):
        if self.redo_stack:
            ...
            self.undo_stack.append((self.shapes, self.eraser_mask.copy(), [list(s) for s in self.eraser_strokes], current_ui_state))
        
            # Load state from redo stack
            shapes, eraser_mask, eraser_strokes, ui_state = self.redo_stack.pop()
            self.shapes = shapes
            self.eraser_mask = eraser_mask
            self.eraser_strokes = [list(s) for s in eraser_strokes]
    ...
    def fill_at_point(self, pt):
        for idx in reversed(range(len(self.shapes))):
            ...
            # Only fill closed shapes or lines
            closed_tools = {"rect", "circle", "triangle", "roundrect", "framewithlabel", "roundframewithlabel"}
            if tool in closed_tools:
                rect = QRect(start, end).normalized()
                if rect.contains(pt):
                    self.push_undo()
                    # Set fill color as 5th element
                    new_shape = list(shape)
                    # Ensure list is long enough
                    while len(new_shape) < 6:
                        new_shape.append(None)
                    new_shape[5] = self.shape_color
                    self.shapes[idx] = tuple(new_shape)
                    return
            elif tool == "line":
                # Check if pt is near the line
                line = (start, end)
                if self._point_near_line(pt, line, tolerance=6):
                    self.push_undo()
                    new_shape = (tool, start, end, self.shape_color, *rest)
                    self.shapes[idx] = new_shape
                    return
            elif tool == "draw":
                points = start
                # Only fill if click is near a point (simulate "filling" a stroke)
                for p in points:
                    if (p - pt).manhattanLength() < 10:
                        self.push_undo()
                        # Set fill color as 5th element (for pen color)
                        new_shape = list(shape)
                        while len(new_shape) < 6:
                            new_shape.append(None)
                        new_shape[5] = self.shape_color
    ...
    def _point_near_line(self, pt, line, tolerance=6):
        """Check if pt is within 'tolerance' pixels of the line segment."""
        p1, p2 = line
        x0, y0 = pt.x(), pt.y()
        x1, y1 = p1.x(), p1.y()
        x2, y2 = p2.x(), p2.y()
        ...
        proj_x = x1 + t * dx
        proj_y = y1 + t * dy
        dist_sq = (x0 - proj_x) ** 2 + (y0 - proj_y) ** 2
        return dist_sq <= tolerance ** 2
    
    def crop_to_rect(self, crop_rect):
        print("Before crop:", self.shapes)
        self.push_undo()
        # 1. Render the entire canvas (A4) at 1:1 scale (no zoom/pan)
        full_canvas = QPixmap(self.a4_size)
        full_canvas.fill(Qt.GlobalColor.transparent)
        ...
        # 2. Crop the rendered canvas pixmap to the crop_rect (which is in canvas coordinates)
        cropped_pixmap = full_canvas.copy(crop_rect)
        

        # 2. "Cut out" the area: erase all shapes and eraser marks inside crop_rect
        # Remove shapes inside crop_rect
        new_shapes = []
        for shape in self.shapes:
            tool, start, end, data = shape[:4]
            extra = shape[4:]
            if tool == "draw":
                # Split polyline into segments outside crop_rect
                segments = self.split_polyline_by_rect(start, crop_rect)
                for seg in segments:
                    new_shape = [tool, seg, None, data]
                    new_shape.extend(extra)
                ...
                if not crop_rect.intersects(rect):
                    ...
                    # For rectangles: subtract crop_rect from rect
                    # Only keep the parts outside crop_rect
                    outside_rects = self.subtract_rect(rect, crop_rect)
                    print("subtract_rect result:", outside_rects)
                    for out_rect in outside_rects:
                        # Always copy all extra fields (fill, rotation, etc.)
                        rotation = shape[4] if len(shape) > 4 and isinstance(shape[4], (int, float)) else None
                        fill_color = shape[5] if len(shape) > 5 and isinstance(shape[5], QColor) else None
                        new_shape = [tool, out_rect.topLeft(), out_rect.bottomRight(), data]
                        if rotation is not None:
                            new_shape.append(rotation)
                        if fill_color is not None:
                            # Ensure fill_color is always the 6th element
                            while len(new_shape) < 5:
                                new_shape.append(None)
                            new_shape.append(fill_color)
                        new_shapes.append(self.normalize_shape_tuple(tuple(new_shape)))
    
        self.shapes = new_shapes

        # Remove eraser strokes inside crop_rect
        self.eraser_strokes = [
            [p for p in stroke if not crop_rect.contains(p)]
            for stroke in self.eraser_strokes
        ]

        # Fill the cropped area with white on the eraser mask (to "clean" the trace)
        painter = QPainter(self.eraser_mask)
        painter.setCompositionMode(QPainter.CompositionMode.CompositionMode_Clear)
        painter.fillRect(crop_rect, Qt.GlobalColor.transparent)
        painter.end()
        
        print("After crop:", self.shapes)
        # Enter adjust-before-placing mode with the cropped image
        self.preview_shape = "image"
    ...
    def normalize_shape_tuple(self, shape):
        """Ensure shape tuple has exactly 6 fields: (tool, start, end, outline_color, rotation, fill_color)"""
        shape = list(shape)
        while len(shape) < 6:
            shape.append(None)
        return tuple(shape)
    
    def subtract_rect(self, rect, crop_rect):
        """Return a list of QRect representing the parts of rect outside crop_rect."""
        result = []
        r = rect
        c = crop_rect.intersected(r)
        if c.isEmpty():
            return [r]
        # Top
        if c.top() > r.top():
            result.append(QRect(r.left(), r.top(), r.width(), c.top() - r.top()))
        # Bottom
        if c.bottom() < r.bottom():
            result.append(QRect(r.left(), c.bottom() + 1, r.width(), r.bottom() - c.bottom()))
        # Left
        if c.left() > r.left():
            result.append(QRect(r.left(), c.top(), c.left() - r.left(), c.height()))
        # Right
        if c.right() < r.right():
            result.append(QRect(c.right() + 1, c.top(), r.right() - c.right(), c.height()))
        # Only non-empty rects
        return [r for r in result if r.width() > 0 and r.height() > 0]
        
    def split_polyline_by_rect(self, points, rect):
        """Split a polyline into segments outside the rect."""
        segments = []
        current = []
        for p in points:
    ...
    def canvas_to_widget(self, pt):
        """Convert canvas QPoint to widget QPoint."""
        center = self.get_canvas_center()
        x = (pt.x() - self.a4_size.width() / 2) * self.scale_factor + center.x() + self.pan_offset.x()
        y = (pt.y() - self.a4_size.height() / 2) * self.scale_factor + center.y() + self.pan_offset.y()
        return QPoint(int(x), int(y))

    def canvas_to_widget_rect(self, rect):
        """Convert canvas QRect to widget QRect."""
        top_left = self.canvas_to_widget(rect.topLeft())
        bottom_right = self.canvas_to_widget(rect.bottomRight())
...
class UIMode(QWidget):
    ...
    def set_tool_category(self, category):
        ...
        pretty_names = {
            ...
            "roundframewithlabel": "Round Frame with Label",
        }
        for tool in category["children"]:
            ...
            if tool["name"] == "eraser":
                btn.doubleClicked.connect(self.show_eraser_size_dialog)
            elif tool["name"] == "draw":
                btn.doubleClicked.connect(self.show_draw_size_dialog)
